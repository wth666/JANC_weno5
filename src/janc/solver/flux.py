...
def weno5_w(U,aux,dx,dy):
    ...
    Y = U[4:,:,:]/U[0:1,:,:]
    Fplus_Y = jnp.concatenate([Fplus, Y], axis=0)
    Fminus_Y = jnp.concatenate([Fminus, Y], axis=0)
    ...
    dGp = WENO_plus_y_w(jnp.concatenate(Gplus_Y)
    dGm = WENO_minus_y_w(jnp.concatenate(Gminus_Y)
